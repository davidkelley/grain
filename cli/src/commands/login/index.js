...
class Login extends CLI {
  execute(alias) {
    let { cli } = this;
    if( ! alias) {
      cli.fatal('--alias is required. Find it here: https://<alias>.signin.aws.amazon.com/');
    } else {
      return new Promise((resolve, reject) => {
        cli.spinner('Opening AWS console..');
        this.url(`http://169.254.169.254`, alias).then((url) => {
  ...
  url(root, alias) {
    return new Promise((resolve, reject) => {
      this.token(root).then((token) => {
        ...
        var destination = encodeURIComponent(CONSOLE_URL);
